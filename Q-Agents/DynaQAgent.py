...
class DynaQAgent(object):
    ...
    def updateQ(self, action, state, reward):
        if self.last_state == None and self.last_action == None:
            ...
            Qmax = max(self.Q[state].values())
            Qsa = self.Q[self.last_state][self.last_action]

            lr = self.learning_rate
            discount = self.discount_factor

            self.Q[self.last_state][self.last_action] += lr * (reward + discount * Qmax - Qsa)
            self.dynaQ(state, action,reward)

            self.last_state = state
            self.last_action = action

    def dynaQ(self,state, action, reward):
        if self.last_state not in self.model:
            self.model[self.last_state] = {}

        self.model[self.last_state][self.last_action] = (reward, state)

        for _ in range(100):
            state = np.random.choice(list(self.model.keys()))
            action = np.random.choice(list(self.model[state]))
            reward, new_state = self.model[state][action]
            Qmax = max(self.Q[new_state].values())
            Qsa = self.Q[state][action]
            lr = self.learning_rate
            discount = self.discount_factor

            self.Q[state][action] += lr * (reward + discount * Qmax - Qsa)
...
if __name__ == '__main__':
    ...
    plt.title("Reward cumulé DynaQ-learning")
    plt.xlabel("iterations")
    plt.ylabel("reward cumulé")
    plt.plot(all_rsum, label="DynaQ-learning")
    plt.legend()
    plt.show()