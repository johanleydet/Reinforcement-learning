...
class QAgent(object):
    ...
    def updateQ(self, obs, next_obs, reward, action): 
        ...
        Qmax=max(self.Q[next_obs,:])
        Qsa=self.Q[obs,action]
        
        lr=self.learning_rate
        discount=self.discount_factor
        
        self.Q[obs, action] += lr * ( reward + discount * Qmax - Qsa )
...
if __name__ == '__main__':
    ...
    for i in range(episode_count):
        ...
        while True:
            ...
            # ici le obs est convertie en nb ce n'est plus une matrice
    ...
    plt.title("Reward cumulé Q_Learning")
    plt.xlabel("iterations")
    plt.ylabel("reward cumulé")
    plt.plot(all_rsum, label="Q_learning")
    plt.legend()
    plt.show()